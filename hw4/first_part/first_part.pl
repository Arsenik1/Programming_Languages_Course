...
delivery_person(dp1, large, [0, 4], none, lecture_hall_a) :- format('Asserted delivery_person: ~w\n', [dp1]).
delivery_person(dp2, medium, [0, 4, 16], none, admin_office) :- format('Asserted delivery_person: ~w\n', [dp2]).
delivery_person(dp3, small, [12, 16], none, cafeteria) :- format('Asserted delivery_person: ~w\n', [dp3]).
delivery_person(dp4, small, [8, 12, 16, 20], none, library) :- format('Asserted delivery_person: ~w\n', [dp4]).
delivery_person(dp5, small, [4, 8, 12, 16, 20], none, social_sciences_bld) :- format('Asserted delivery_person: ~w\n', [dp5]).
delivery_person(dp6, small, [0, 4, 8, 12, 16, 20], none, institute_y) :- format('Asserted delivery_person: ~w\n', [dp6]).
delivery_person(dp7, small, [0, 8, 16], none, engineering_bld) :- format('Asserted delivery_person: ~w\n', [dp7]).
delivery_person(dp8, small, [ 12, 16], none, cafeteria) :- format('Asserted delivery_person: ~w\n', [dp8]).
delivery_person(dp9, small, [0, 4, 8], none, library) :- format('Asserted delivery_person: ~w\n', [dp9]).
delivery_person(dp10, small, [0, 4, 8, 12, 16, 20], none, admin_office) :- format('Asserted delivery_person: ~w\n', [dp10]).
...
% Calculate the total work hours based on the number of 4-hour blocks
sum_work_hours(WorkBlocks, TotalHours) :-
    length(WorkBlocks, NumberOfBlocks),
    TotalHours is NumberOfBlocks * 4.

% Check if the total delivery time is within the total work hours
check_work_hours(WorkBlocks, TotalTime) :-
    sum_work_hours(WorkBlocks, TotalHours),
